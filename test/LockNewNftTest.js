...
describe("Uniswap V3 Lockers", function () {
    ...
    async function deployFixture() {
        ...
        return { owner, bob, dustReceiver, additionalCollector, weth, uncx, tokenA, tokenB, univ3locker, pool, nftId_unlocked, NftPositionManager };
    }

    describe("New Lock Testing", function () {

        it("Should fail to allow to lock someone elses NFT", async function () {
            const { univ3locker, owner, bob, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager } = await loadFixture(deployFixture);
            var lockParams = [
            ...
            await expect(univ3locker.connect(bob).lock(lockParams)).to.be.revertedWith('ERC721: transfer of token that is not own')
        });

        it("Should fail to allow to lock with an unwhitelisted nftpositionmanager address", async function () {
            const { univ3locker, owner, uncx, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager } = await loadFixture(deployFixture);
            var lockParams = [
                uncx.target,
                nftId_unlocked,
                dustReceiver.address, // receiver of collect() and tokens which dont fit in liquidity
            ...
            await expect(univ3locker.connect(owner).lock(lockParams)).to.be.revertedWith("INVALID NFT POSITION MANAGER")
        });
        ...
        it("Should fail to allow to lock with a date greater than 1e10 (incase someone mistakenly enters a date in miliseconds)", async function () {
            ...
            await expect(univ3locker.connect(owner).lock(lockParams)).to.be.revertedWith('MILLISECONDS')
        });
        ...
        it("Should fail on invalid country code", async function () {
            ...
            await expect(univ3locker.connect(owner).lock(lockParams)).to.be.revertedWith('COUNTRY')
        });
        ...
        it("Should store a lock with the same params as what was sent to the lock method", async function () {
            ...
            var lockA = {
                ...
                countryCode: Number(lock.countryCode)
            }
            var lockB = {
        ...
        it("Should check lock nonces and nft_id integrity", async function () {
            const { univ3locker, owner, bob, weth, uncx, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager } = await loadFixture(deployFixture);
            ...
            var nftId1 = BigInt(await NftPositionManager.connect(owner).tokenOfOwnerByIndex(owner.address, 1))
            var nftId2 = BigInt(await NftPositionManager.connect(owner).tokenOfOwnerByIndex(owner.address, 2))
            var nftId3 = BigInt(await NftPositionManager.connect(owner).tokenOfOwnerByIndex(owner.address, 3))

            expect(nftId1).to.equal(nftId2 - BigInt(1))
            expect(nftId2).to.equal(nftId3 - BigInt(1))

            await NftPositionManager.connect(owner).approve(univ3locker.target, nftId1)
            await NftPositionManager.connect(owner).approve(univ3locker.target, nftId2)
            await NftPositionManager.connect(owner).approve(univ3locker.target, nftId3)
        ...
        it("Should check events", async function () {
            const { univ3locker, pool, tokenA, tokenB, owner, bob, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager } = await loadFixture(deployFixture);
            var lockParams = [
                NftPositionManager.target,
                nftId_unlocked,
                dustReceiver.address, // receiver of collect() and tokens which dont fit in liquidity
            ...
            var call = univ3locker.connect(owner).lock(lockParams)
