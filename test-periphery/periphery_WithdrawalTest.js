...
describe("Uniswap V3 Lockers", function () {
    ...
    describe("Withdraw lock Testing", function () {
        ...
        it("Should fail to allow to withdraw someone elses NFT", async function () {
            const { univ3locker, fullRangeConvertor, owner, bob, additionalCollector, dustReceiver, nftId_unlocked_1, NftPositionManager } = await loadFixture(deployFixture);
        ...
        it("Should collect fees on withdraw to collectFeeAddress if they chose a fee option with collection fees", async function () {
            const { univ3locker, fullRangeConvertor, owner, bob, weth, uncx, tokenA, tokenB, additionalCollector, dustReceiver, nftId_unlocked_1, NftPositionManager, SwapRouter, autoCollector, lpFeeReceiver, collectFeeReceiver } = await loadFixture(deployFixture);
        ...
        it("Should zero the lock state after withdrawl", async function () {
            const { univ3locker, fullRangeConvertor, owner, bob, weth, uncx, additionalCollector, dustReceiver, nftId_unlocked_1, NftPositionManager, SwapRouter, autoCollector, lpFeeReceiver, collectFeeReceiver } = await loadFixture(deployFixture);
        ...
        it("Should reduce getNumUserLocks count by one for on chain pagination", async function () {
            const { univ3locker, fullRangeConvertor, owner, bob, weth, uncx, additionalCollector, dustReceiver, nftId_unlocked_1, nftId_unlocked_2, nftId_unlocked_3, NftPositionManager, SwapRouter, autoCollector, lpFeeReceiver, collectFeeReceiver } = await loadFixture(deployFixture);
