...
describe("Uniswap V3 Lockers", function () {
    ...
    describe("Decrease Liquidity Testing", function () {
        ...
        it("Should allow to decrease a locked positions liquidity if unlock date has elapsed", async function () {
            const { univ3locker, fullRangeConvertor, owner, bob, additionalCollector, collectFeeReceiver, dustReceiver, weth, uncx, nftId_unlocked_1, NftPositionManager } = await loadFixture(deployFixture);
            ...
            expect(await weth.balanceOf(bob.address)).to.equal((liquidityBefore.amount0 / BigInt(2)) - BigInt(1))
            expect(await uncx.balanceOf(bob.address)).to.equal((liquidityBefore.amount1 / BigInt(2)) - BigInt(1))
            expect(await weth.balanceOf(collectFeeReceiver.address)).to.equal(0)
            expect(await uncx.balanceOf(collectFeeReceiver.address)).to.equal(0)
