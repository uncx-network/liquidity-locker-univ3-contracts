...
describe("Uniswap V3 Lockers", function () {
  ...
  async function deployFixture() {
      ...
      var FLAT_FEE = 200
      await univ3locker.addOrEditFee("DEFAULT", 50, 200, FLAT_FEE, ethers.ZeroAddress);
      // FLAT_FEE = 0
      ...
      return { owner, bob, dustReceiver, additionalCollector, weth, uncx, tokenA, tokenB, univ3locker, fullRangeConvertor, pool, nftId_unlocked, NftPositionManager, FLAT_FEE };
  }

  describe("New Lock with gas Testing", function () {

      it("Should fail to allow to lock someone elses NFT", async function () {
          const { univ3locker, fullRangeConvertor, owner, bob, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager, FLAT_FEE } = await loadFixture(deployFixture);
          var lockParams = [
          ...
          await expect(fullRangeConvertor.connect(bob).convertToFullRangeAndLock(lockParams, [1,1,0,0]), {value: FLAT_FEE}).to.be.revertedWith('ERC721: transfer of token that is not own')
      });

      it("Should fail to allow to lock with an unwhitelisted nftpositionmanager address", async function () {
          const { univ3locker, fullRangeConvertor, owner, uncx, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager, FLAT_FEE } = await loadFixture(deployFixture);
          var lockParams = [
          ...
          await expect(fullRangeConvertor.connect(owner).convertToFullRangeAndLock(lockParams, [1,1,0,0]), {value: FLAT_FEE}).to.be.revertedWithoutReason()
      });
      ...
      it("Should fail to allow to lock with a date greater than 1e10 (incase someone mistakenly enters a date in miliseconds)", async function () {
          ...
          await expect(fullRangeConvertor.connect(owner).convertToFullRangeAndLock(lockParams, [1,1,0,0]), {value: FLAT_FEE}).to.be.revertedWith('MILLISECONDS')
      });
      ...
      it("Should fail on invalid country code", async function () {
          ...
          await expect(fullRangeConvertor.connect(owner).convertToFullRangeAndLock(lockParams, [1,1,0,0]), {value: FLAT_FEE}).to.be.revertedWith('COUNTRY')
      });
      ...
      it("Should store a lock with the same params as what was sent to the lock method", async function () {
          ...
          await fullRangeConvertor.connect(owner).convertToFullRangeAndLock(lockParams, [1,1,0,0], {value: FLAT_FEE})
      ...
      it("Should check lock nonces and nft_id integrity", async function () {
          const { univ3locker, fullRangeConvertor, owner, bob, weth, uncx, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager, FLAT_FEE } = await loadFixture(deployFixture);
      ...
      it("Should check events", async function () {
          const { univ3locker, fullRangeConvertor, pool, tokenA, tokenB, owner, bob, additionalCollector, dustReceiver, nftId_unlocked, NftPositionManager, FLAT_FEE } = await loadFixture(deployFixture);
          var lockParams = [
              NftPositionManager.target, 
              nftId_unlocked, 
              dustReceiver.address, // receiver of collect() and tokens which dont fit in liquidity
          ...
          var call = fullRangeConvertor.connect(owner).convertToFullRangeAndLock(lockParams, [1,1,0,0], {value: FLAT_FEE})
